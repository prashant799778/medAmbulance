package com.example.drivermedambulance.Comman;

import android.view.View;

import com.example.drivermedambulance.rangebar.Bar;

public class URLS {
    public static String BASE_URL="http://134.209.153.34:5077/";

    ////////////////////////GET_METHODS/////////////////////////
    ///////////////////////POST_METHODS/////////////////////////
    public static String ADD_USER=BASE_URL+"addUser";
    public static  String LOGIN=BASE_URL+"Login";
    public  static String selectambulanceMaster=BASE_URL+"selectambulanceMaster";
    public  static String selectambulanceTypeMaster=BASE_URL+"selectambulanceTypeMaster";
    public static String USERSIGNUP =BASE_URL+"userSignup";
    public static String VERIFY =BASE_URL+"verifyOtp";
    public static String MyTrips =BASE_URL+"myTrip";
    public static String Responder_Trip =BASE_URL+"responderTrip";
    public static String YOUR_RIDE =BASE_URL+"myrides";
    public static String userProfile =BASE_URL+"userProfile";
    public static String userUpdateProfile =BASE_URL+"updateuserProfile";

    public static String DriverProfile =BASE_URL+"driverProfile";
    public static String DriverUpdateProfile =BASE_URL+"updateDriverProfile";
    public static String AboutUs =BASE_URL+"allaboutUs";
    public static String AddLicence=BASE_URL+"addDrivertest";
    public static  String responderProfile=BASE_URL+"responderProfile";
    public static String updateresponderProfile=BASE_URL+"updateresponderProfile";
    public static String Support=BASE_URL+"support";
    public static String driverSignup=BASE_URL+"driverSignup";
    public static String driverLogin=BASE_URL+"driverLogin";
    public static String addDriverRating=BASE_URL+"addDriverRating";
    public static String updateDriverProfile111=BASE_URL+"updateDriverProfile";

}
